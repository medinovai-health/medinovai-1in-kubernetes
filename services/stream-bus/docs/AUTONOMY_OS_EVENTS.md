# AutonomyOS Event Wiring — medinovai-real-time-stream-bus
...
```json
{
  ...
  "eventVersion": "1.0",
  "source":       "medinovai-real-time-stream-bus",
  "tenantId":     "uuid",
  "correlationId": "uuid",
  "timestamp":    "ISO8601",
  "payload":      {}
...
# Publisher
mos_publisher = EventPublisher(broker_url=E_BROKER_URL, service="medinovai-real-time-stream-bus")
...
mos_subscriber = EventSubscriber(
    ...
    queue="medinovai-real-time-stream-bus.events",
    dlq=f"{E_DLQ_PREFIX}medinovai-real-time-stream-bus.events",
...
Failed messages go to `DLQ.medinovai-real-time-stream-bus.events`:
