# AutonomyOS Event Wiring — medinovai-deploy
...
# Publisher
mos_publisher = EventPublisher(broker_url=E_BROKER_URL, service="medinovai-deploy")
...
mos_subscriber = EventSubscriber(
    ...
    queue="medinovai-deploy.events",
    dlq=f"{E_DLQ_PREFIX}medinovai-deploy.events",
...
Failed messages go to `DLQ.medinovai-deploy.events`:
