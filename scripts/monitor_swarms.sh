#!/bin/bash

# MedinovAI Swarm Monitoring Script
# This script provides real-time monitoring of agent swarms

set -euo pipefail

# Configuration
LOG_DIR="swarm_logs"
SWARM_CONFIG_DIR="swarm_configs"
REFRESH_INTERVAL=10

# Colors for output
RED='\033[0;31m'
GREEN='\033[0;32m'
YELLOW='\033[1;33m'
BLUE='\033[0;34m'
PURPLE='\033[0;35m'
CYAN='\033[0;36m'
NC='\033[0m' # No Color

# Logging functions
log_info() {
    echo -e "${BLUE}ℹ️  $1${NC}"
}

log_success() {
    echo -e "${GREEN}✅ $1${NC}"
}

log_warning() {
    echo -e "${YELLOW}⚠️  $1${NC}"
}

log_error() {
    echo -e "${RED}❌ $1${NC}"
}

log_swarm() {
    echo -e "${PURPLE}🤖 $1${NC}"
}

log_coordinator() {
    echo -e "${CYAN}🎯 $1${NC}"
}

# Get swarm status
get_swarm_status() {
    local swarm_id="$1"
    local status_file="$LOG_DIR/swarm_${swarm_id}_status.json"
    
    if [[ -f "$status_file" ]]; then
        jq -r '.status' "$status_file" 2>/dev/null || echo "unknown"
    else
        echo "not_started"
    fi
}

# Get agent status
get_agent_status() {
    local swarm_id="$1"
    local agent_id="$2"
    local status_file="$LOG_DIR/swarm_${swarm_id}_agent_${agent_id}_status.json"
    
    if [[ -f "$status_file" ]]; then
        local status
        status=$(jq -r '.status' "$status_file" 2>/dev/null || echo "unknown")
        local success
        success=$(jq -r '.success_count' "$status_file" 2>/dev/null || echo "0")
        local failed
        failed=$(jq -r '.failed_count' "$status_file" 2>/dev/null || echo "0")
        echo "$status:$success:$failed"
    else
        echo "not_started:0:0"
    fi
}

# Display swarm dashboard
display_dashboard() {
    clear
    echo "🎯 MedinovAI Agent Swarms Dashboard"
    echo "==================================="
    echo "Last Updated: $(date)"
    echo ""
    
    # Get all swarm configurations
    local swarm_count=0
    local total_repos=0
    local total_success=0
    local total_failed=0
    local active_swarms=0
    local completed_swarms=0
    
    for config_file in "$SWARM_CONFIG_DIR"/swarm_*_config.json; do
        if [[ -f "$config_file" ]]; then
            local swarm_id
            swarm_id=$(jq -r '.swarm_id' "$config_file")
            local phase
            phase=$(jq -r '.phase' "$config_file")
            local repo_count
            repo_count=$(jq -r '.repositories | length' "$config_file")
            
            ((swarm_count++))
            total_repos=$((total_repos + repo_count))
            
            # Get swarm status
            local swarm_status
            swarm_status=$(get_swarm_status "$swarm_id")
            
            case "$swarm_status" in
                "running")
                    echo -e "${YELLOW}🤖 Swarm $swarm_id ($phase): RUNNING${NC}"
                    ((active_swarms++))
                    ;;
                "completed")
                    echo -e "${GREEN}✅ Swarm $swarm_id ($phase): COMPLETED${NC}"
                    ((completed_swarms++))
                    ;;
                "failed")
                    echo -e "${RED}❌ Swarm $swarm_id ($phase): FAILED${NC}"
                    ;;
                *)
                    echo -e "${BLUE}⏳ Swarm $swarm_id ($phase): NOT STARTED${NC}"
                    ;;
            esac
            
            # Show agent details
            for agent_id in $(seq 1 5); do
                local agent_status_info
                agent_status_info=$(get_agent_status "$swarm_id" "$agent_id")
                local agent_status
                agent_status=$(echo "$agent_status_info" | cut -d: -f1)
                local agent_success
                agent_success=$(echo "$agent_status_info" | cut -d: -f2)
                local agent_failed
                agent_failed=$(echo "$agent_status_info" | cut -d: -f3)
                
                case "$agent_status" in
                    "completed")
                        echo -e "  ${GREEN}  ✅ Agent $agent_id: $agent_success successful, $agent_failed failed${NC}"
                        total_success=$((total_success + agent_success))
                        total_failed=$((total_failed + agent_failed))
                        ;;
                    "running")
                        echo -e "  ${YELLOW}  🔄 Agent $agent_id: Running...${NC}"
                        ;;
                    "failed")
                        echo -e "  ${RED}  ❌ Agent $agent_id: Failed${NC}"
                        ;;
                    *)
                        echo -e "  ${BLUE}  ⏳ Agent $agent_id: Not started${NC}"
                        ;;
                esac
            done
            echo ""
        fi
    done
    
    # Display summary
    echo "📊 Summary:"
    echo "  🤖 Total Swarms: $swarm_count"
    echo "  🔄 Active Swarms: $active_swarms"
    echo "  ✅ Completed Swarms: $completed_swarms"
    echo "  📁 Total Repositories: $total_repos"
    echo "  ✅ Successful: $total_success"
    echo "  ❌ Failed: $total_failed"
    
    if [[ $total_repos -gt 0 ]]; then
        local success_rate
        success_rate=$(echo "scale=1; $total_success * 100 / $total_repos" | bc -l)
        echo "  📈 Success Rate: ${success_rate}%"
    fi
    
    echo ""
    echo "Press Ctrl+C to exit monitoring"
}

# Monitor swarms continuously
monitor_continuous() {
    log_info "Starting continuous monitoring (refresh every ${REFRESH_INTERVAL}s)"
    
    while true; do
        display_dashboard
        sleep "$REFRESH_INTERVAL"
    done
}

# Show swarm logs
show_swarm_logs() {
    local swarm_id="$1"
    local log_file="$LOG_DIR/swarm_${swarm_id}.log"
    
    if [[ -f "$log_file" ]]; then
        echo "📋 Swarm $swarm_id Logs:"
        echo "========================"
        tail -50 "$log_file"
    else
        log_warning "No logs found for Swarm $swarm_id"
    fi
}

# Show agent logs
show_agent_logs() {
    local swarm_id="$1"
    local agent_id="$2"
    local log_file="$LOG_DIR/swarm_${swarm_id}_agent_${agent_id}.log"
    
    if [[ -f "$log_file" ]]; then
        echo "📋 Swarm $swarm_id, Agent $agent_id Logs:"
        echo "========================================"
        tail -50 "$log_file"
    else
        log_warning "No logs found for Swarm $swarm_id, Agent $agent_id"
    fi
}

# Show execution report
show_execution_report() {
    local report_file="$LOG_DIR/execution_report.json"
    
    if [[ -f "$report_file" ]]; then
        echo "📊 Execution Report:"
        echo "==================="
        jq '.' "$report_file"
    else
        log_warning "No execution report found"
    fi
}

# Main execution
main() {
    case "${1:-dashboard}" in
        "dashboard")
            display_dashboard
            ;;
        "monitor")
            monitor_continuous
            ;;
        "logs")
            if [[ -n "${2:-}" ]]; then
                show_swarm_logs "$2"
            else
                log_error "Please specify swarm ID: $0 logs <swarm_id>"
                exit 1
            fi
            ;;
        "agent-logs")
            if [[ -n "${2:-}" && -n "${3:-}" ]]; then
                show_agent_logs "$2" "$3"
            else
                log_error "Please specify swarm ID and agent ID: $0 agent-logs <swarm_id> <agent_id>"
                exit 1
            fi
            ;;
        "report")
            show_execution_report
            ;;
        "help")
            echo "MedinovAI Swarm Monitoring"
            echo "========================="
            echo ""
            echo "Usage: $0 [COMMAND] [OPTIONS]"
            echo ""
            echo "Commands:"
            echo "  dashboard     Show current swarm status (default)"
            echo "  monitor       Monitor swarms continuously"
            echo "  logs <id>     Show logs for specific swarm"
            echo "  agent-logs <swarm_id> <agent_id>  Show logs for specific agent"
            echo "  report        Show execution report"
            echo "  help          Show this help"
            echo ""
            echo "Examples:"
            echo "  $0                    # Show dashboard"
            echo "  $0 monitor           # Continuous monitoring"
            echo "  $0 logs 1            # Show logs for swarm 1"
            echo "  $0 agent-logs 1 2    # Show logs for swarm 1, agent 2"
            echo "  $0 report            # Show execution report"
            ;;
        *)
            log_error "Unknown command: $1"
            echo "Use '$0 help' for usage information"
            exit 1
            ;;
    esac
}

# Run main function
main "$@"








