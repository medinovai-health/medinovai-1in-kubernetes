#!/bin/bash
# migrate_integration_services.sh - Migrate Integration services from medinovaios to medinovai-integration-services
...
# List of Integration services to migrate
...
echo "Services to migrate: ${#INTEGRATION_SERVICES[@]}"
echo ""
        ...
        cat > "$target_path/service-info.json" << EOF
{
    ...
    "target_repository": "medinovai-integration-services",
    "description": "Integration service migrated from medinovaios"
}
EOF
...
# Migrate all Integration services
SUCCESS_COUNT=0
TOTAL_COUNT=${#INTEGRATION_SERVICES[@]}

echo "🚀 Starting migration of $TOTAL_COUNT Integration services..."
echo ""
...
# Commit changes
git commit -m "Migrate $SUCCESS_COUNT Integration services from medinovaios

- Migrated services: ${INTEGRATION_SERVICES[*]}
- Migration date: $(date -u +%Y-%m-%dT%H:%M:%SZ)
- Source: medinovaios
- Target: medinovai-integration-services"
