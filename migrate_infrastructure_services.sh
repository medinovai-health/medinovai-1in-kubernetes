#!/bin/bash
# migrate_infrastructure_services.sh - Migrate Infrastructure services from medinovaios to medinovai-infrastructure

echo "🏗️ Starting Infrastructure services migration..."
echo "Timestamp: $(date)"
echo "=========================================="
...
# List of Infrastructure services to migrate
...
echo "Services to migrate: ${#INFRASTRUCTURE_SERVICES[@]}"
echo ""
        ...
        cat > "$target_path/service-info.json" << EOF
{
    ...
    "target_repository": "medinovai-infrastructure",
    "description": "Infrastructure service migrated from medinovaios"
}
EOF
...
# Migrate all Infrastructure services
SUCCESS_COUNT=0
TOTAL_COUNT=${#INFRASTRUCTURE_SERVICES[@]}

echo "🚀 Starting migration of $TOTAL_COUNT Infrastructure services..."
echo ""
...
# Commit changes
git commit -m "Migrate $SUCCESS_COUNT Infrastructure services from medinovaios

- Migrated services: ${INFRASTRUCTURE_SERVICES[*]}
- Migration date: $(date -u +%Y-%m-%dT%H:%M:%SZ)
- Source: medinovaios
- Target: medinovai-infrastructure"
